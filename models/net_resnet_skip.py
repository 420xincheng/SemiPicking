...
class StdConv1d(nn.Conv1d):
    def forward(self, x):
        w = self.weight  # torch.cuda.FloatTensor
        v, m = torch.var_mean(w, dim=[1, 2], keepdim=True, unbiased=False)  # 返回方差和均值
        w = (w - m) / torch.sqrt(v + 1e-5)  # 权重标准化  减去均值，除以标准差
        x = x.to(torch.float32)
        return F.conv1d(x, w, self.bias, self.stride, self.padding, self.dilation, self.groups)


# stride=1时， H,W不变   stride=2时，H，W减半
def conv3x3(cin, cout, stride=1, groups=1, bias=False):
    return StdConv1d(cin, cout, kernel_size=3, stride=stride, padding=1, bias=bias, groups=groups)


# stride=1时：不改变H,W，
def conv1x1(cin, cout, stride=1, bias=False):
    return StdConv1d(cin, cout, kernel_size=1, stride=stride, padding=0, bias=bias)
...
class ResNetV2(nn.Module):
    ...
    def __init__(self, block_units, width_factor):
        super().__init__()
        width = int(64 * width_factor)  # 输入的channel数 - 64
        self.width = width

        self.pool = nn.MaxPool1d(kernel_size=3, stride=2, padding=1)
        self.root = nn.Sequential(OrderedDict([
            ('conv', StdConv1d(32, width, kernel_size=7, stride=2, bias=False, padding=3)),  # L 减半
    ...
    def forward(self, x):
        features = []  # 保存特征值  x = [1,32,4096]
        x = self.root(x)  # -> [1, 64, 2048] L -> L / 2 = 2048
        features.append(x)
        x = self.pool(x)  # -> [1, 64, 1024] L -> L / 2
        for i in range(len(self.body)):
            x = self.body[i](x)
            feat = x
            features.append(feat)
        return x, features[::-1]  # x通道数由32 -> 1024  长度由L -> L/16=256
